...
let style_class = {
	filled:
		'variant-filled-primary bg-primary text-white shadow-lg rounded-full px-[1.5rem] py-[0.5rem]',
	clean:
		'variant-text-primary bg-white shadow-lg rounded-full px-[1.5rem] py-[0.5rem]',
	ghost: ''
}[type];
...
<button
	on:click={() => onClick()}
	bind:this={element}
	class={`${style_class} ${style} opacity-0`}
	disabled={disabled}
>
